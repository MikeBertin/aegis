...
![tests](https://img.shields.io/badge/tests-51_passing-3fb950)
...
| PID step response (tuned in simulation) |
|:--:|
| ![step](docs/media/pid_step.gif) |
| 22° step acquired in **0.67 s**, ~1% overshoot, 0.16° steady-state. |
...
pytest                                     # 51 headless tests, no GPU needed
```
...
```
Three panels: **PID tuner** (live response), **turret tracking sim**, and the **safety-gate playground**. (To publish it as a live URL, make the repo public and enable GitHub Pages on `docs/site`.)
